...
int qry(int u,int v){
	...
	while(up[u]!=up[v])if(dep[up[u]]>dep[up[v]])
		ans=max(ans,tr.qry(pos[up[u]],pos[u])),u=fa[up[u]];
	else ans=max(ans,tr.qry(pos[up[v]],pos[v])),v=fa[up[v]];
	if(pos[u]>pos[v])swap(u,v);
	return max(ans,tr.qry(pos[u],pos[v]));
}
void ins(int u,int v,int w){
	while(up[u]!=up[v])if(dep[up[u]]>dep[up[v]])
		tr.ins(pos[up[u]],pos[u],w),u=fa[up[u]];
	else tr.ins(pos[up[v]],pos[v],w),v=fa[up[v]];
	if(pos[u]>pos[v])swap(u,v);
	tr.ins(pos[u],pos[v],w);
}
int main(){
	scanf("%d%d",&n,&Q);tr.build(n);
	for(int i=1;i<n;i++){
