...
uses math,windows;
const n=15;
      maxv:array[1..14]of longint=(10,8,7,7,7,7,6,6,6,6,6,6,6,6);
      lian=3;jian=2;t=1;
      sh=2;chong=1;huo=3;
      inf=1000000;
...
var x,y,i,j,i1,j1,m1,br:longint;
    ...
    maxnode:dword=800000;code:integer;
...
begin
   y:=1 xor x;
   if (a=x)and(b<0)and(c=x)and(d=x)and(e<0)and(f=x)then exit(0);
...
begin
    y:=1 xor x;
    if(a<0)and(b<0)and(c<0)and(d=x)and(e=x)and(f<0)and(g=y)or
      (a<0)and(b<0)and(c=x)and(d=x)and(e<0)and(f<0)or
      (a=y)and(b<0)and(c=x)and(d=x)and(e<0)and(f<0)and(g<0)
      then exit(lian);
    if(a<0)and(b=x)and(c<0)and(d=x)and(e<0)and(f<0)or
      (a<0)and(b<0)and(c=x)and(d<0)and(e=x)and(f<0)and(g=y)
      then exit(jian);
...
var c3_p,c3_q,i4_p,i4_q,x,q,h3_p,h3_q,i,j,c:longint;
begin
    inc(tot);
    q:=1 xor p;x:=0;
    h3_p:=0;c3_p:=0;i4_p:=0;
    h3_q:=0;c3_q:=0;i4_q:=0;
    //p's value
    for i:=-1 to n+2 do
      begin
    ...
    for i:=2 to n-1 do
      for j:=2 to n-1 do
        begin
          ...
          if a[i+1,j-1]>=0 then inc(c);
          if a[i+1,j+1]>=0 then inc(c);
          if (a[i,j]=q)and(c<4) then dec(x,3);
        end;
    for i:=-1 to n+2 do
      begin
    ...
    if (i4_p=chong)or(h3_p=1) then inc(x,18);
    dec(x,c3_q*13);
    inc(x,c3_p*4);
...
var i,j,k,v,sum,dt,tdep,u,mul:longint;tmp:array[1..4]of longint;
...
begin
        ...
        if check(i,j) then
          begin
            ...
            while (k>1) and ( //TOO HARD!!!
             ...
             (s[k-1,4]>s[k,4]) and ((s[k-1,1]<s[k,1]-30)xor xorb) or
             (s[k-1,4]<s[k,4]) and ((s[k-1,1]<s[k,1]+30)xor xorb)) do begin
    ...
    if (s[1,1]<-5000)or(s[1,1]>5000) then begin
    ...
    for i:=1 to u do
      begin
        ...
        if(p=1)and(s[i,1]<-5000)then break;
        if(p=0)and(s[i,1]>5000)then break;
        a[s[i,2],s[i,3]]:=p;
        if d<maxd then begin
            h:=hs[i];
            tdep:=ceil((u-i+1)/u*(maxd-d)+d);
            if (zhb[h.sh].val<1000000)and(zhb[h.sh].key=h.lh)and
               ...
               then v:=zhb[h.sh].val
                       ...
                       if (tdep<maxd)and(br=0)and((p=1)and(v>alpha)or(p=0)and(v<beta)) then begin
                          dt:=1;v:=minimax(maxd,d+1,p xor 1,alpha,beta);
                       end;
                       if br=0 then begin
                          zhb[h.sh].val:=v;
                          zhb[h.sh].key:=h.lh;
                          if dt=1
                            then zhb[h.sh].dep:=maxd-d
                            else zhb[h.sh].dep:=tdep-d;
                       end;
                    end;
          end else v:=s[i,1];
        a[s[i,2],s[i,3]]:=-1;
