...
var s:ansistring;fl:string;ch:char;
    i,l,len,totwd,maxl,sk:longint;stime:dword;
...
begin
    ...
    repeat
        ...
        if (zy[1][it.key]>1)and(nhd[it.key]>l/400) then
...
begin
    ...
    close(input);
    sort1(1,totwd);
    for i:=1 to totwd do begin
        if pos(#13,wds[i])<>0 then begin inc(sk);dec(totwd);continue;end;
        write(wds[i]:20,' ',wps[i]:4);
        if (i-sk) mod 3=0 then writeln;
    end;
    writeln;
    writeln('Found ',totwd,' words. Time used : ',(gettickcount()-stime)/1000:0:3,' (s)');
    while 1=1 do
end.
