...
int tocomp(decimal a,comp *c,int &Flen){
	...
	if(_l==MAXLEN && a.intt){
		...
		for(int i=0;i<_l-1;i++)
			c[i]=((int)(c[i].real()+0.5))/pow2+((int)(c[i+1].real()+0.5))%pow2*pow1;
		c[_l-1]=((int)(c[_l-1].real()+0.5))/pow2+a.intt*pow1;
		c[_l]=0;Flen=(_l<<2)-sf;
		return MAXLEN;
	}else{
		Flen=_l<<2;
...
decimal operator * (decimal a,decimal b){
	int len1,len2,len=ceil(log2(tocomp(a,_c1,len1)+tocomp(b,_c2,len2)));
	fft(_c1,len);fft(_c2,len);
	for(int i=0;i<(1<<len);i++)_c1[i]=conj(_c1[i]*_c2[i]);
	fft(_c1,len);len=1<<len;
	for(int i=0;i<len;i++)_r[i]=_c1[i].real()/len+0.5;
